...
const TarotCircle = {
  ...
  _bindEvents(container) {
    container.addEventListener('mousedown', (e) => {
      this.isDragging = true; this.prevMouseX = e.clientX; this.rotationSpeed = 0;
    });
    ...
    window.addEventListener('mouseup', (e) => {
      if (!this.isDragging) return;
      const dx = Math.abs(e.clientX - this.prevMouseX);
      this.isDragging = false;
      if (dx < 4 && Math.abs(this.rotationSpeed) < 0.008) this._trySelectCard(e);
    });
  ...
  _trySelectCard(e) {
    ...
    if (intersects.length > 0) {
      ...
      burstParticles(e.clientX, e.clientY, 25);
      this.updateProgress();
